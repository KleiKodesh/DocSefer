...
{
    public class AlignColumnsBottom
    {
        class ColumnObject
        {
        ...
        void Apply()
        {
            using (new UndoRecordHelper("יישור טורים"))
            {
                ...
                for (int i = pageData.FirstPage; i <= pageData.LastPage; i++)
                {
                    ...
                    {
                        int breakPoint = sectionRange.ColumnBreakPoint();

                        Range column1 = document.Range(sectionRange.Start, breakPoint - 1);
                        ...
                        Range column2 = document.Range(breakPoint, sectionRange.End);
                        Range column2Bottom = document.Range(sectionRange.End, sectionRange.End);
                        float column2_y = (float)column2Bottom.Information[WdInformation.wdVerticalPositionRelativeToPage];

                        if (column1_y == column2_y)
                            continue;
                        ...
                        AlignColumns(columns.OrderBy(c => c.yPos).ToList());
                    }
                }
            }
        }

        void AlignColumns(List<ColumnObject> columns)
        {
